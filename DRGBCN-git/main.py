...
def train(config, model_cls=Tmodel):
    time_stamp = time.asctime()
    time_stamp = "_".join(time_stamp.split()).replace(":", "-")
    ...
    for split_id, datamodule in enumerate(cv_spliter):
        ...
        # poltting(labels, score, split_id, 10)
        if config.debug:
            break
...
def parse(print_help=False, model_cls=Tmodel):
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", default="Tmodel", type=str)
    parser.add_argument("--epochs", default=100, type=int)
    parser.add_argument("--drug_feature_topk", default=16, type=int)  # add prior knowledge
    parser.add_argument("--disease_feature_topk", default=16, type=int)
