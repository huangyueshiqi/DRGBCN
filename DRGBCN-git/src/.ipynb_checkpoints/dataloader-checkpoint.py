...
class DRDataset():
    def __init__(self, dataset_name="Fdataset", drug_neighbor_num=15, disease_neighbor_num=15):
        assert dataset_name in ["Cdataset", "Fdataset", "DNdataset", "lrssl", "hdvd", "lagcn"]
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument("--dataset_name", default="Fdataset",
                                   choices=["Cdataset", "Fdataset", "lrssl", "hdvd", "DNdataset", "lagcn"])
        parser.add_argument("--drug_neighbor_num", default=3, type=int)
        parser.add_argument("--disease_neighbor_num", default=3, type=int)
...
class CVDataset(pl.LightningDataModule):
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument("--train_fill_unknown", default=False, action="store_true", help="训练集中是否将测试集关联填0还是丢弃，默认：丢弃")
    ...
    def prepare_data(self):
        ...
        self.train_mask = data["train_mask"].astype('bool')
        self.test_mask = data["test_mask"].astype('bool')
...
def load_LAGCN(root_dir="dataset/lagcn"):
    ...
    """
    dd = pd.read_csv(os.path.join(root_dir, "dis_sim.csv"), header=None).to_numpy(dtype=np.float32)
    rd = pd.read_csv(os.path.join(root_dir, "drug_dis.csv"), header=None).to_numpy(dtype=np.float32)
    rr = pd.read_csv(os.path.join(root_dir, "drug_sim.csv"), header=None).to_numpy(dtype=np.float32)
