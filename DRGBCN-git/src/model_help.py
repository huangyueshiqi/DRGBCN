...
class BaseModel(pl.LightningModule):
    ...
    def get_epoch_loss(self, outputs):
        ...
        loss_info = {key: [output[key].detach().cpu() for output in outputs ] for key in loss_keys}
        loss_info = {key: sum(value)/len(value) for key, value in loss_info.items() if not torch.isnan(sum(value))}
        return loss_info

    def training_epoch_end(self, outputs):  #tensorborad
        stage = "train"
